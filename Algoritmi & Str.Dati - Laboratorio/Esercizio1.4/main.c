...
double singleExperiment(int length, int maxInstances, int alg)
{
    ...
    for (i = 0; i < maxInstances; i++)
    {
        ...
        else if (alg == 5) // QS tail ricorsivo
        {
            t_start = clock();

            tailQuickSort(array, 0, length - 1);
...
void experiment(int minLength, int maxLength)
{
    ...
    int maxInstances = 5;
    int step = 5;
    double timeIS, timeMS, timeHS, timeQS_M3, timeSQ_Tail;
    int length;
    ...
    for (length = minLength; length <= maxLength; length += step)
    {
        ...
        timeSQ_Tail = singleExperiment(length, maxInstances, 5);

        // IS; MS; HS; QS_M3; DIM
        printf(" %f; %f; %f; %f; %f; %d\n", timeIS, timeMS, timeHS, timeQS_M3, timeSQ_Tail, length);
    }
}
